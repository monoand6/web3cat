...
def test_read_write(call: Call, calls_repo: CallsRepo):
    ...
    calls = list(
        calls_repo.find(c1.address, c1.calldata, c1.block_number, c1.block_number)
    )
    assert len(calls) == 0
    calls = list(
        calls_repo.find(c1.address, c1.calldata, c1.block_number, c1.block_number + 1)
    )
    assert calls == [c1]
    calls = list(
        calls_repo.find(
    ...
    assert calls == [c2]
    calls = list(
        calls_repo.find(c1.address, c1.calldata, c1.block_number, c1.block_number + 11)
    )
